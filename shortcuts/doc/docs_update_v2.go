...
func v2UpdateFlags() []common.Flag {
	return []common.Flag{
		{Name: "command", Desc: "operation; requirements: str_replace(--pattern), block_delete(--block-id, comma-separated for batch), block_insert_after/block_replace(--block-id,--content), block_copy_insert_after/block_move_after(--block-id,--src-block-ids), overwrite/append(--content). append always inserts at document end (equiv. block_insert_after --block-id -1)", Enum: validCommandsV2Keys()},
		...
		{Name: "block-id", Desc: "target block ID(s) for block operations (comma-separated for batch delete); -1 means document end (same as --command append). Use explicit block IDs for chapter-by-chapter insertion"},
